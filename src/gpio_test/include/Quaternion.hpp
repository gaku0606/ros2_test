...
* @brief  クォータニオンを使えるクラスです．
*/

class Quaternion{
public:
	/**
		@brief	Quaternionインスタンスを生成します
	*/
	Quaternion(){
		w = 1.0f;
		x = 0.0f;
	...
	/**
	*  @brief Vector3クラスからクォータニオンを作ります
	*/
	...
	/**
	*  @brief   クォータニオンを回転軸と回転角度によって初期化します。
...
public:
	...
	*/
	...
	/**
		@brief	クォータニオンを正規化して，単位クォータニオンにします．
	...
	*/
	...
	/**
	*  @brief   クォータニオンをVector3クラスで初期化します。
	*/
	...
	*/
	...
	/**
	*  @brief   クォータニオンの各要素に配列のようにアクセスします
	*/
	double q(int i){
		double ans = 0.0;
		switch (i){
	...
	*/
	...
	/**
	*  @brief  2つの３次元ベクトルを一致させるクォータニオンを計算
	...
	*/
	...
	/**
	*  @brief  クォータニオンをVector3クラスに変換します
	...
	*/
	void Rotation(Vector3* v) {
		if (v == NULL) return;
		static double ww = 0.0f;
		static double xx = 0.0f;
		static double yy = 0.0f;
		static double zz = 0.0f;

		static double vx = 0.0f, vy = 0.0f, vz = 0.0f;
		static double _wx, _wy, _wz, _xy, _zx, _yz;
		ww = w * w;
		xx = x * x;
		yy = y * y;
		zz = z * z;
	...
	*/
	void InvRotation(Vector3* v) {
		...
		static double vx = 0.0f, vy = 0.0f, vz = 0.0f;
		static double _wx, _wy, _wz, _xy, _xz, _yz;
		ww = w * w;
		xx = x * x;
		yy = y * y;
		zz = z * z;
...
void Quaternion::FromToRotation(Vector3 from, Vector3 to){
	double halfTheta = 0.5f * from.Angle(to);//回転角度 0からpi/2
...
* @brief クォータニオンの掛け算をします．この際，順序が重要です．
*/
